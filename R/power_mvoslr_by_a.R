#' Estimation of power of group-sequential multivariate one-sample log-rank test via simulation with varying accrual durations
#'
...
power_mvoslr_by_a <- function(reference_model, events, analysis_dates, accrual_durations,
  ...
  for(i in 1:simulation_runs){
    ...
    sim_frame <- msm_to_trial_data(sim_frame, longest_accrual,
                                   final_analysis - longest_accrual)

    # Collect results of simulated trial
    result <- execution_mvoslr_by_a(msm_data = sim_frame, analysis_dates = analysis_dates, current_analysis = num_analyses,
...
power_mvoslr_by_a_par <- function(reference_model, events, analysis_dates, accrual_durations,
  ...
  if(!requireNamespace("foreach", quietly = TRUE) |
    ...
    results_list <- foreach::foreach(i = 1:cores, .combine = list) %dopar% {
       ...
       result_loc <- power_mvoslr_by_a(reference_model = reference_model, events = events,
                                       analysis_dates = analysis_dates, accrual_durations = accrual_durations,
                                       recruitment_speed = recruitment_speed, hazard_ratios = hazard_ratios,
                                       cum_hazard_functions_alternative = cum_hazard_functions_alternative,
                                       norm = norm, boundaries = boundaries, alpha = alpha, weights = weights,
