#' Execution of interim and final analyses of multivariate group-sequential one-sample log-rank test with varying sample sizes
#'
#' For a given data set, this function can efficiently compute the results of the group-sequential multivariate one-sample
#' log-rank test for different choices of sample sizes. This function is needed to efficiently estimate the power of such
#' trials for different sample sizes via simulation.
#'
...
#' @param sample_sizes Vector of sample sizes that shall be investigated
...
#' @keywords internal
#'
execution_mvoslr_by_n <- function(msm_data, analysis_dates, accrual_duration, current_analysis = NULL,
                                  reference_model, events, sample_sizes,
                                  norm = "l2", boundaries = "obf", alpha = 0.05, weights = NULL){
  ...
  if(!model_type %in% c("M", "SM")){
    ...
    events_count  <- array(0, c(number_event_types, number_event_types, current_analysis, number_sample_sizes))
    events_hazard <- array(0, c(number_event_types, number_event_types, current_analysis, number_sample_sizes))

    events_count_inc  <- array(0, c(number_event_types, number_event_types, current_analysis, number_sample_sizes))
    events_hazard_inc <- array(0, c(number_event_types, number_event_types, current_analysis, number_sample_sizes))

    # Create matrices/vectors for martingale, test statistics and p-values
    mv_martingale <- array(0, dim = c(number_event_types, current_analysis, number_sample_sizes))
    mv_martingale_inc <- array(0, dim = c(number_event_types, current_analysis, number_sample_sizes))
    mv_test_statistic <- array(0, dim = c(number_event_types, current_analysis, number_sample_sizes))
    uv_test_statistic <- matrix(0, nrow = current_analysis, ncol = number_sample_sizes)
    ...
    # Find patients to analyse/include for different sample sizes
    # Each sample size gets a list of patient ids to include into analysis for that sample size
    # Sets are chosen s.t. they are monotone increasing
    sample_sizes <- sort(sample_sizes)
    temp_ids <- unique(msm_data$id)
    patient_ids <- list(temp_ids)
    for(n_index in number_sample_sizes:1){
      temp_ids <- sample(temp_ids, sample_sizes[n_index], replace = FALSE)
      patient_ids <- c(list(temp_ids), patient_ids)
    }
      ...
      for(i in 1:number_event_types){
        ...
        for(j in i:number_event_types){
          ...
          if(length(intersect_event) > 0){
            ...
            for(n_index in 1:number_sample_sizes){

              # Note: to compute results for several sample sizes, we use the randomly chosen patients sampled above
              # Need "_temp" object for computation for different sample sizes
              relevant_obs_temp <- relevant_obs[which(relevant_obs$Tstop <= event_times_rep &
                                                        relevant_obs$to %in% intersect_event &
                                                        relevant_obs$id %in% patient_ids[[n_index]]), ]

              sum_intersect_events <- sum(relevant_obs_temp$status)
              sum_accumulated_hazard <- sum(relevant_obs_temp$acc_haz)

              events_count[i, j, number_of_analysis, n_index] <-
                events_count[j, i, number_of_analysis, n_index]  <- sum_intersect_events
              events_hazard[i, j, number_of_analysis, n_index] <-
                events_hazard[j, i, number_of_analysis, n_index] <- sum_accumulated_hazard
    ...
    standardisation_matrices <- array(standardisation_matrices_raw,
                                      dim = c(number_event_types, number_event_types, current_analysis, number_sample_sizes))

    mv_test_statistic <- array(0, dim=c(number_event_types, current_analysis, number_sample_sizes))
    for(i in 1:current_analysis){
      for(n_index in 1:number_sample_sizes){
        mv_test_statistic[, i, n_index] <- standardisation_matrices[, , i, n_index] %*% mv_martingale_inc[, i, n_index]
      }
    }
    ...
    p_cum <- matrix(0, nrow = current_analysis, ncol = number_sample_sizes)

    for(i in 1:current_analysis){
      # Need to apply matrix function for the special case i=1, otherwise this object is not recognized as a matrix
      p_cum[i, ] <- apply(matrix(stagewise_p_values[1:i, ], nrow = i, ncol = number_sample_sizes), 2, inverse_normal_combine)
    }

    # Determine rejection
    rejection <- p_cum <= matrix(rep(levels[1:current_analysis], number_sample_sizes), ncol = number_sample_sizes)
    colnames(rejection) <- paste("n=", sample_sizes, sep = "")
